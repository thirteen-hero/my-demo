...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
    ...
    // compilation类内部会维护一个全局唯一的moduleGraph实例对象
    // 每次解析出新模块后,将module、dependency以及模块之间的关系moduleConnection记录到compilation.moduleGraph对象中
	...
	_buildModule(module, callback) {
        ...
        // 实际进行模块构建 根据模块类型使用自己的build方法进行构建 如NormalModule
				module.build(
					this.options,
					this,
    ...
    // 使用给定的工厂函数创建模块
        ...
        // 将新模块添加到编译过程中
          ...
          // 在这里获取到需要编译的模块 后续的流程都会根据模块类型分别进行处理
					const module =
    ...
    // 尝试从dependencyFactories中获取相应的模块工厂
  ...
  // 封装chunk
    ...
    // 创建chunkGraph 是模块和chunks之间关系的核心数据
    // 初始化chunkGraph实例,确定哪些模块属于哪个chunk,以及chunks之间如何相互引用
    ...
    // 遍历入口点,对于配置中定义的每个入口点执行以下步骤：
    // 1、创建chunk: 为每个入口点创建一个新的chunk 这个chunk将作为从该入口点构建出的所有模块的容器
    // 2、创建并设置entryPoint: 为每个chunk创建一个entryPoint对象,确保了每个入口点都有一个对应的chunk,且该chunk包含了入口点及其依赖的所有模块
    // 3、处理依赖关系: 将入口点的直接和间接依赖的模块与创建的chunk进行关联
		for (const [name, { dependencies, includeDependencies, options }] of this
			.entries) {
      // 为每个入口点创建一个chunk
			const chunk = this.addChunk(name);
			if (options.filename) {
        // chunk的filenameTemplate设置为配置的filename
				chunk.filenameTemplate = options.filename;
			}
      // 创建entrypoint对象并设置其对应的chunk
      ...
      // 处理入口点直接和间接依赖的模块
          ...
          // 记录模块到chunkGraphInit中,为后续构建chunk grouph做准备
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
    ...
    // 使用收集的信息(入口点、依赖等) 构建完整的chunkGraph
	...
	Make sure to select an appropriate stage from Compilation.PROCESS_ASSETS_STAGE_*.`
                ...
                // 生成chunkAssets 将chunk转换成输出的assets
                // entry chunks
                // 规则: 每个入口点至少生成一个entry chunk
                // 目的: 确保应用或页面的入口有一个对应的chunk,包含所有必要的启动代码
                // 配置: 通过entry配置指定
                // async chunks
                // 规则: 使用import()语句导入的模块会被分装到一个新的async chunk中
                // 目的: 实现代码拆分和懒加载,优化初始加载时间,按需加载额外功能
                // 配置: 无需特殊配置,webpack自动处理动态导入
	...
	createChunkAssets(callback) {
		...
		asyncLib.forEachLimit(
			...
			(chunk, callback) => {
				...
				let manifest;
				try {
          // 获取chunk将要生成的assets的清单,触发renderManifest钩子
