...
const concurrentLimit = async(
  ...
  timeout: number = 3000,
    ...
    for (const item of requestList) {
      ...
      // 创建带重试的任务
      const promise = retryPromise((signal) => Promise.resolve(request({signal, ...item})),retry, delay, timeout)
        .then((res) => {
          // 释放坑位
 ...
 */
const retryPromise = (
  // 传入的是一个箭头函数,每次执行时都会创建一个新的promise,去发送request请求
      ...
      taskFn(signal)
        ...
        .catch((error: Error) => {
          ...
          if (restRetry === 0) {
            ...
            // 等待一段时间后递归重试
            // 使用 delay * 2^已重试次数 实现指数退避（1s, 2s, 4s...)
            ...
            const retried = maxRetry - restRetry;
            const exponentialDelay = delay * Math.pow(2, retried);
